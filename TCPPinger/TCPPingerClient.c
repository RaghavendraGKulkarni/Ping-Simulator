...
int main(int argc, char **argv) {
    ...
    int socketFD, messageLength, numberOfPings, packetLossCount;
    ...
    // Try creating a client docket and print appropriate message
    ...
    // Try connecting the client socket to the server socket and print appropriate message
    ...
    for(int i = 1; i <= numberOfPings; i++) {   
        ...
        listen:
            messageLength = read(socketFD, buffer, sizeof(buffer));
            end = clock();
